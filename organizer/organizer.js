...
async function add_metada_show(library) {
    for (var entry in library) {
        ...
        for (var candidate in candidates.results) {
            ...
            var distance = sift3Distance(
                esn.clean(entry.toLowerCase()),
                esn.clean(name.toLowerCase())
            );
            if (distance < min || min == -1) {
                min = distance;
                id = candidates.results[candidate].id;
...
function gen_library_show(tree) {
    ...
    for (var entry in tree) {
        ...
        for (sub_entry in tree[entry]) {
            ...
            if (
                typeof tree[entry][sub_entry][Object.keys(tree[entry][sub_entry])[0]] ==
                "object"
            ) {
                ...
                for (var file in tree[entry][sub_entry]) {
                    ...
                    library[entry].content[details.episode][file_details.episode] =
                        tree[entry][sub_entry][file][0];
                }
            } else {
                ...
                library[entry].content[file_details.season][file_details.episode] =
                    tree[entry][sub_entry][0];
            }
        }
    }
    return library;
...
function get_tree(folder) {
    ...
    filesystem.readdirSync(folder).forEach(function (file) {
        ...
        var new_name = folder + "/" + file.replace(/[ _]/g, "-");
