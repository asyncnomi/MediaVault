...
module.exports.get_potential_show = async function (str) {
    return await get_potential_show(str)
};
...
async function get_potential_show(name) {
  // TMDB don't handle underscore in search request
  name = name.replace("_", "-")
  let candidates = await request("https://api.themoviedb.org/3/search/tv?api_key="+api_key+"&query="+name)
  return candidates
}

async function get_meta_show(id) {
